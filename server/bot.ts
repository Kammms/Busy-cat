  ...
  private async handleInteraction(interaction: ChatInputCommandInteraction) {
    ...
    if (commandName === 'set') {
      ...
      if (sub === 'team') {
        ...
        await interaction.reply({ content: `✅ Moderator team role set to <@&${role.id}>`, flags: 64 });
      } else if (sub === 'track') {
        const channel = options.getChannel('channel', true);
        await storage.updateSetting(SETTINGS_KEYS.TRACKED_CHANNEL_ID, channel.id);
        await interaction.reply({ content: `✅ Tracking channel set to <#${channel.id}>`, flags: 64 });
      } else if (sub === 'gender') {
        ...
        await interaction.reply({ content: `✅ Point values updated.`, flags: 64 });
      }
      ...
      await interaction.reply({ content: `✅ <@${user.id}> has been excluded from tracking.`, flags: 64 });
      ...
      if (moderator) {
        await storage.updateModerator(moderator.id, { isIgnored: false });
        await interaction.reply({ content: `✅ <@${user.id}> has been included back in tracking.`, flags: 64 });
      } else {
        await interaction.reply({ content: `❌ <@${user.id}> is not in the system. They will be added automatically when they send a message in the tracked channel.`, flags: 64 });
      }
      ...
      let mod = await storage.getModeratorByDiscordId(user.id);
      if (!mod) {
        return interaction.reply({ content: "❌ This user is not a tracked moderator.", flags: 64 });
      }
      ...
      if (type === 'messages') {
        if (amount === null) return interaction.reply({ content: '❌ Please provide an `amount` for this type.', flags: 64 });
        ...
        if (rank) {
          ...
          return interaction.reply({ content: '❌ Please provide either a `rank` or an `amount` for leaderboard points.', flags: 64 });
        }
      ...
      if (!mod) {
        return interaction.reply({ content: "❌ You are not a tracked moderator.", flags: 64 });
      }
      ...
      try {
        ...
        const requiredRoles = [role1, role2, role3].filter(Boolean) as NonNullable<typeof role1>[];
        ...
        // Use mention syntax for content text, plain names for embed titles (mentions are invalid in embed titles)
        const roleMentions = requiredRoles.map(r => `<@&${r.id}>`).join(' + ');
        const roleNames = requiredRoles.map(r => r.name).join(' + ');
        if (matching.size === 0) {
          await interaction.editReply({ content: `No members found with all of: ${roleMentions}` });
          return;
        }
        ...
        const embeds = trimmed.map((chunk, i) => {
          ...
          if (i === 0) embed.setTitle(`👥 Members with ${roleNames} (${matching.size})`);
          return embed;
        });
      ...
      if (sub === 'add') {
        ...
        await interaction.reply({ content: `✅ Rank **${name}** added with ID **${rank.id}** (Requires **${points}** points).`, flags: 64 });
      } else if (sub === 'modify') {
        ...
        if (!rank) return interaction.reply({ content: `❌ Rank with ID **${id}** not found. Use \`/ranks list\` to see valid IDs.`, flags: 64 });
        ...
        await interaction.reply({ content: `✅ Rank **${id}** updated.`, flags: 64 });
      } else if (sub === 'remove') {
        const id = options.getInteger('id', true);
        const existing = await storage.getModRanks();
        const rank = existing.find(r => r.id === id);
        if (!rank) return interaction.reply({ content: `❌ Rank with ID **${id}** not found.`, flags: 64 });
        
        await storage.deleteModRank(id);
        await interaction.reply({ content: `✅ Rank **${id}** (**${rank.name}**) removed.`, flags: 64 });
      } else if (sub === 'list') {
        const ranks = await storage.getModRanks();
        if (ranks.length === 0) return interaction.reply({ content: "No ranks configured. Use \`/ranks add\` to create one.", flags: 64 });
